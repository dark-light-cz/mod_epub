...
#define MGX_EPUB_MIME_TYPE "application/epub+zip"
#define ngx_http_epub_current_file(ctx) ctx->pieces[ctx->pieces_i].file
...
} ngx_http_epub_piece_e;

typedef struct {
    ngx_http_epub_range_t    range;
    ngx_http_epub_file_t    *file;
    ngx_http_epub_piece_e    type;
} ngx_http_epub_piece_t;

typedef struct {
    ngx_array_t             unparsed_request;
    ngx_http_epub_piece_t   *pieces;
    ngx_array_t             files;
    ngx_array_t             ranges;
    ngx_uint_t              ranges_i;
    ngx_uint_t              pieces_i;
...
typedef struct {
    ngx_http_epub_file_t    *requesting_file;
} ngx_http_epub_sr_ctx_t;
